    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Interpolators i) : SV_Target
            {
                ...
                intersection *= saturate(saturate(sin(2 * _Time.z + 8 * (1-intersection))) + smoothstep(0.9, 1, intersection));
               

                // refraction effect
                float2 distortedScreenUV = screenUV + (float2(0.1, 0.4) * (w * saturate(i.normal.y) * _refractionIntensity));
